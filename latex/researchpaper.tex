...
[\textbf{done?: mentioning how generative model factors and mentioning fig 1 again.}] A generative model described in figure \ref{simple_draw} at a month $t$ can be written as follows given some a priori assumptions:

\begin{equation}
\label{gen_model}
\left.\begin{aligned}
p(\theta) & =p(\mu,\sigma^2,\lambda_t, N, O_t, p_{(A,t)}, n_{(A,t)}, x_{(A,t)}, U_t) \\
...
The chain rule for random variables \cite{chain_rule} is used to the joint distribution of all variables at the first line of equation \ref{gen_model} such that the second line of the equation is derived.
