...
\normalsize
[\textbf{phrase differently: the prior and the data relate to all the unknown parameter, but as a last stage hou focuz on one target.}]
...
\normalsize 
The first simulation assumed $N= 10000, n_{A}=1000$.  \\
[\textbf{Q;Before moving on to simulation, you should write out how the generative(generable) model factors, and mention Fig1 again.}]
...
\normalsize 
There exist two data sets: the survey data ($n_A, x_A$), and the ambulance attended overdose data ($U_{1:T}$). The two data sets were simulated as follows. 
It was assumed that the two data sets were collected for a year (T=12). The true value of $p_A$ was set as $p_A=0.8$ and the survey sample size $n_A$ was set as $n_A=1000$ for every months [one survey or 12?]. The $x_A$ values for each month were independentally generated from the Binomial distribution (\ref{ambulance}).  In terms of the overdose data, it is assumed that the true values of parameters for overdose model were $\mu=\log0.05,\ \sigma=1$.
The vector of $O_t$ was generated following the overdose model (\ref{overdose}). The vector of $U_t$ was generated from the Binomial relation of the two variables ($\ref{over_amb}$). The two generated vectors have the same length with the survey data (t=1,2,3, ..., 12).  Note that only $U_t$ and $x_A$ are known as the likelihood [intended meaning?] and $p_A$ needs to be estimated first in order to estimate $O_t$ which is the ultimate interest of the research.\\
...
Figure \ref{hst_ot} is the histogram of posterior samples of total overdoses per year ($O_1 + O_2 + ... + O_{12}$). 2000 samples were collected and shown as the histogram.  It is seen that the plot is bell shaped and not skewed. The total number of overdoses per year is around 10900 to 11000. [mention about credible interval] \\
