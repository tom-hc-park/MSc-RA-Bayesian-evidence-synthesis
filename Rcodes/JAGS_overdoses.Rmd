...
# z_t <- z lmb_t <- lambda
# length unification: z_t is length 12 vector
...
ggplot()+
  ...
  geom_boxplot(aes(x=month,y=o_t), color='grey',alpha = 0.01,data = trace_o_t)+
...
ggplot()+geom_boxplot(aes(x=month,y=u_t),data = df_u_t)+geom_point(aes(x=u_t_values$month, y=u_t_values$u_t),color='red')
```
