...
```
# Uncertainty of N
Now we suppose the population size of BC, N, is unknown. We may have some information about the population size and transform it into a prior distribution. Let's assume from the point that we don't have a good understanding of the sample size. But N should be at least bigger than the sample size of survey, the ambulance attended overdoses. We have 12 data points of $U_t$. We also have 12 data points of $x_A$. 

$$N \geq max(\vec{U_t}, \vec{x_A})$$

Where $\vec{U_t}, \vec{x_A}$ are the vectors of $U_t$ and $x_A$.

Then, we need a theoretical maximum value of N. BC is a province of Canada. For now, since I do not have a good idea about the maximum possible number of N, let's just say two times of the real value is the maximum. 

$$N \in  \{a, a+1, a+2, ..., b\} \\ \textbf{where } a= \max(\vec{U_t}, n_A), b= 2 \times \textbf{real value of  N}$$

However, in the model file below, we will use a prior such that 
$$N \in  \{0,1,2, ..., b\} \\ \textbf{where }  b= 2 \times \textbf{real value of  N}$$ since we don't know how to construct such prior as described below. 
...
```{r}
test <- coda.samples(model = simple.N.model,variable.names = c('o_t','p_a','N'), n.iter = 1000)
interations = 1000
burnin= floor(interations/2)
...
```
```{r}
ggplot(data = as.data.frame(N), aes(x= N)) +geom_histogram()+ geom_vline(xintercept =10000)
```

It took a lot of time to fit the bayesian model this time. We have overestimated N value whose mode is around 17000 while the real value is 10000. Unif(0,20000) has 10000 as its mean. I guess the b was too large. 
I will wait for the reply from Paul. 
```{r}
plot(test)
```
