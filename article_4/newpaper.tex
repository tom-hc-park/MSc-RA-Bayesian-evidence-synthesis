...
\end{equation}
where $\theta$ represents the target of estimation, the total overdoses and $y$ represents the collected data set providing relevant samples of the target variable. In Bayesian concepts, the variable of interest $\theta$ is a fixed number but the number is yielded from a certain distribution; That is, $\theta$ is considered to be a sample of a random variable. 

The problem, however, is that the data set $y$ is not available. Hence the technique is to approach the estimation in a indirect fashion. Figure \cite{simple_drawing} shows how we estimate the target variable. 
\\
...
[brief explaination of MCMC and related topics]
